      ...
      {items.map((item) => {
        ...
        return (
          <button
            ...
            type='button'
            className={`text-md lg:text-lg md:text-md sm:text-sm bg-transparent text-light relative leading-6 font-lobster flex justify-center items-center rounded-full focus:outline-none p-2 mx-2 hover:brightness-125 ${isSelected ? 'brightness-125': 'brightness-85'}`}
            disabled={false}
          >
            {item.name}
                { isSelected ?   (
